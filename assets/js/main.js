...
(function($) {
   ...
   if(navclose.length){
       ...
       if ($.fn.onePageNav) {
           $("#onepage-menu .nav-menu").onePageNav({
    ...
    $(window).on( 'load', function() {
        $("#site-load").delay(1000).fadeOut(500);
    ...
    if(rs_pie.length) {
        $('.rs-pie').easyPieChart({
            size: 70,
            barColor: getComputedStyle(document.documentElement).getPropertyValue("--brand-yellow").trim() || "#ffd600",
            scaleLength: 0,
            lineWidth: 5,
            trackColor: "#eeeeee",
            lineCap: "circle",
    ...
    // Counter numbers stay static (no count-up animation)
   ...
   $(document).ready(function(){
       if (!$("#mobile-navbar-menu .submenu-button").length) {
